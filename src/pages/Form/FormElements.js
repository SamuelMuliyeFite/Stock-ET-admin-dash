...
const TableOne = () => {
  ...
  const accept=((companyEmail,companyName,password,verified)=>{
      ...
      <div className="flex flex-col">
        ...
        <div className="grid grid-cols-3 border-b border-stroke dark:border-strokedark sm:grid-cols-5">
         {signuprequest && signuprequest.map((item)=>(
          ...
          <div className="flex items-center justify-center p-2.5 xl:p-5">
            <p className="text-meta-3">{item.verified}</p>
          </div>
          <button onClick={()=>{accept(item.companyEmail,item.password,item.verified,item.companyName)}} className="inline-flex items-center justify-center rounded-md border border-meta-3 py-4 px-10 text-center font-medium text-meta-3 hover:bg-opacity-90 lg:px-8 xl:px-10">
